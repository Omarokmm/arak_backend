...
const getCasesByMonth = async (req, res) => {
  ...
  if (startDate && endDate) {
    try {
      ...
      // Top Priority Cases In All Cases
      const topPriorityCases = await Case.find({
        isTopPriority: true,
      ...
      return res.status(responsesStatus.OK).json({
        ...
        holdingCases: holdingCases,
        urgentCases: urgentCases,
        topPriorityCases: topPriorityCases,
        studyCases: studyCases,
        redoCases: redoCases,
        count: cases.length,
      });
...
const createCase = async (req, res) => {
  ...
  try {
    ...
    const newCase = await Case.create({
      ...
      isInvoice,
      isEmail,
      isPhoto: (photos && photos.length > 0) ? true : false,
      photos,
      deadline,
      deadlineCeramic,
      deadlineFitting,
...
const updateCase = async (req, res) => {
  ...
  try {
    ...
    if (updateData.photos) {
      updateData.isPhoto = updateData.photos.length > 0;
    }
    const buffCase = await Case.findByIdAndUpdate({ _id: id }, updateData, { new: true });
    if (!buffCase) {
      return res
        .status(responsesStatus.BadRequest)
  ...
  try {
    ...
    let updateQuery = {
      $set: {
        ...
        [`isTopPriority`]: updateFields.isTopPriority,
...
const updateIsTopPriorityCase = async (req, res) => {
  const { id, isTopPriority } = req.params;
  const buffHistoryTopPriority = req.body;
  try {
    // First, find the document to check if `historyHolding` exists
    const existingCase = await Case.findById(id);
    ...
    const updatedCase = await Case.findByIdAndUpdate(
      id,
      existingCase.historyTopPriority
        ? {
          $set: {
            ["isTopPriority"]: isTopPriority,
            ["historyTopPriority"]: req.body,
          },
        }
        : {
          $set: { ["isTopPriority"]: isTopPriority },
          $push: { historyTopPriority: { $each: buffHistoryTopPriority } }, // Append new items to existing array
        },
      { new: true }
    );
    res.json(updatedCase);
...
const deleteFile = async (req, res) => {
  try {
    ...
    // Cloudinary destroy API for images/videos
    // Note: for videos you would ideally specify resource_type optionally, but destroy often figures it out or defaults to image.
    // Actually, Cloudinary destroy requires resource_type for videos:
